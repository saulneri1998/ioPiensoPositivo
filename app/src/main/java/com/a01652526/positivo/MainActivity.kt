package com.a01652526.positivo
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Toast.makeText(this, "Pienso un poco positivo", Toast.LENGTH_LONG).show()
    }

    fun next(view: View) {
        textView_positivo.text = getRandomMessage()
    }

    fun getRandomMessage(): String {
        val r = Random.nextInt(0, database.size - 1)
        this.index = r
    ...
    fun nextMessage(view: View) {
        index++
        index = index % database.size
        textView_positivo.text = database[index]
    }

    fun previousMessage(view: View) {
        index--
        index = index % database.size
        if (index < 0) index = database.size-1
        textView_positivo.text = database[index]
    }

    fun searchMessage(view: View) {
        val searchIndex = editText.text.toString().toInt()
        if (searchIndex < 0 || searchIndex > database.size-1) {
            Toast.makeText(this, "Index out of range", Toast.LENGTH_LONG).show()
        } else {
            index = searchIndex
            textView_positivo.text = database[searchIndex]
        }
